...
async function getProperty<T = any>(
  propInterface: ClientInterface,
  property: string,
): Promise<T> {
  const { value }: dbus.Variant = await propInterface
...
async function updateSong(name: string, p: ClientInterface) {
  try {
    ...
    const artwork = metadata
      ? await getArtwork(metadata["mpris:artUrl"]?.value)
      : "";

    const playerName = getPlayerName(name);

    const length = metadata
      ? Number(metadata["mpris:length"]?.value) / 1000000 || 0
      : 0;
...
async function updatePostion(name: string, p: ClientInterface) {
  try {
    ...
    await updateEww(
      "positions",
      JSON.stringify(Object.fromEntries(postitions)),
    );
  } catch (err) {
    console.error(err);
  }
...
async function watchPlayer(name: string) {
  try {
    const playerObject = await bus.getProxyObject(
      name,
      "/org/mpris/MediaPlayer2",
    );
    const propInterface = playerObject.getInterface(
    ...
    propInterface.on("PropertiesChanged", () =>
      updateSong(name, propInterface),
    );
  } catch (err) {
    console.error(err);
